...
def main():
    ...
    torch_module.load_state_dict(torch.load(r"./pretrained/mobilenetv2_jester_online.pth.tar"))   # 加载模型，路径自己修改
    ...
    while True:  # 读取摄像头
        ...
        if i_frame % 2 == 0:  # skip every other frame to obtain a suitable frame rate ， 隔帧抽取
            ...
            # output_shapes = {}
            # with torch.no_grad():
            #     # torch_inputs = shift_buffer
            #     # torch_inputs.insert(0, input_var)
            #     for index, torch_input in enumerate(shift_buffer):
            #         name = "i" + str(index)
            #         input_names.append(name)
            #         input_shapes[name] = torch_input.shape
            #
            #         out_name = "o" + str(index)
            #         output_names.append(out_name)
            #         output_shapes[out_name] = torch_input.shape
            #
            #     input_names.insert(0,'input')
            #     output_names.insert(0,'output')
            #     torch.onnx.export(torch_module, (input_var, *shift_buffer), './pretrained/mobilenetv2_jester_online.onnx', input_names=input_names,
            ...
            with torch.no_grad():
                ...
                feat, *shift_buffer = torch_module(input_var, *shift_buffer)
